...
# ── API Keys — load from Streamlit Secrets (cloud) or fall back to empty ────
...
with st.sidebar:
    ...
    # Keys load automatically from Streamlit Secrets — override here if needed
    eia_key_override = st.text_input("EIA API Key (optional override)", type="password", placeholder="leave blank to use configured key")
    fred_key_override = st.text_input("FRED API Key (optional override)", type="password", placeholder="leave blank to use configured key")
    if eia_key_override: eia_key = eia_key_override
    if fred_key_override: fred_key = fred_key_override
